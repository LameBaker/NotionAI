# Current Iteration

## Objective
Execute Task 1 from `docs/plans/2026-03-12-notionai-mvp-implementation.md` using TDD.

## Tasks
- [x] Finalize MVP architecture and ACL model.
- [x] Write design document in `docs/plans/`.
- [ ] Validate real Notion root IDs for `HR` and `Development`.
- [ ] Validate Google OU paths for pilot users.
- [ ] Create implementation plan file for coding session.
- [ ] Implement Task 1 runtime skeleton (`app/__init__.py`, `app/config.py`, `app/models.py`, `tests/test_config.py`).

## Exit Criteria
- Task 1 tests are added and passing.
- Task 1 implementation commit is created.
